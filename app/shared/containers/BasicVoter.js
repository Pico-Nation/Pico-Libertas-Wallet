...
class BasicVoterContainer extends Component<Props> {
  ...
  tick() {
    ...
    if (validate.NODE === 'SUCCESS') {
      ...
      if (settings.blockchain.tokenSymbol === "PICO") {
        getPriceFeedGecko("TELOS", "USD", settings.blockchain.tokenSymbol);
        getPriceFeedGecko("TELOS", "PICO", settings.blockchain.tokenSymbol);
      }
      else {
        getPriceFeedGecko(settings.blockchain.tokenSymbol, "USD");
        getPriceFeedGecko(settings.blockchain.tokenSymbol, "PICO");
      }
    }
    ...
    if (
      ...
      settings.account) {
       ...
       pico(connection).getAccount(settings.account).then((results) => {
        ...
        if (claimer.voter_info && claimer.voter_info.last_claim_time) {
          ...
          if (secondsSince > 86401) {
            (async () => {
              ...
              setTimeout(() => {
                pico(connection).getActions(settings.account, -1, -10).then((results) => {
                    const {
                      cpu_weight,
                      net_weight

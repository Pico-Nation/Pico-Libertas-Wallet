...
export function importWallet(account, 
  ...
  return (dispatch: () => void, getState) => {
    ...
    if (!pubkey && accountData) {
      const auths = new PICOAccount(accountData).getKeysForAuthorization(authorization);
      if (auths.length > 0) {
        ([{ pubkey }] = auths);
      }
    }
...
export function upgradeWallet(chainId, account, authorization, password = false, swap = false) {
  return (dispatch: () => void, getState) => {
    ...
    if (current.length > 0) {
      pico(connection).getAccount(account).then((accountData) => {
        ...
        const derived = new PICOAccount(accountData).getAuthorization(pubkey);
...
export function upgradeWatchWallet(chainId, account, authorization, swap = false) {
  return (dispatch: () => void, getState) => {
    ...
    if (current.length > 0) {
      pico(connection).getAccount(account).then((accountData) => {
        const model = new PICOAccount(accountData);

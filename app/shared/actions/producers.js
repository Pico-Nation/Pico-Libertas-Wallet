...
export function getProducers(previous = false) {
  return (dispatch: () => void, getState) => {
    ...
    pico(connection).getTableRows(query).then((results) => {
      ...
      if (contract && contract['picoio.token']) {
        // fixed values for PICO and TELOS chains until there's a
        // better way to find these dynamically via PICOIO
        const coreSymbol = settings.blockchain.tokenSymbol;
        const isPICOChain = (coreSymbol === 'PICO');
        const coreToken = contract['picoio.token'][coreSymbol];
        const supply = coreToken.supply ? parseFloat(coreToken.supply) : 0;
        // yearly inflation
        const inflation = isPICOChain ? 0.04879 : 0.025;
        // Tokens per year
        const tokensPerYear = supply * inflation;
        // Tokens per day
        const tokensPerDay = tokensPerYear / 365;
        // % of inflation
        const tokensToProducers = isPICOChain ? (tokensPerDay * 0.2) : (tokensPerDay * 0.1);
      ...
      const defaultPubKey = 'PICO1111111111111111111111111111111114T1Anm';
      const data = rows

...
import PICOAccount from '../utils/PICO/Account';
...
export function checkAccountAvailability(account = '') {
  return (dispatch: () => void, getState) => {
    ...
    if (account && (settings.node || settings.node.length !== 0)) {
      pico(connection).getAccount(account).then(() => dispatch({
...
export function getAccount(account = '') {
  return (dispatch: () => void, getState) => {
    ...
    if (account && (settings.node || settings.node.length !== 0)) {
      pico(connection).getAccount(account).then((results) => {
        ...
        if (settings.account === account) {
          ...
          const model = new PICOAccount(results);
          if (model) {
...
export function getActions(account, start, offset) {
  return (dispatch: () => void, getState) => {
    ...
    if (account && (settings.node || settings.node.length !== 0)) {
      pico(connection).getActions(account, start, offset).then((results) => {
...
export function getCurrencyBalance(account, requestedTokens = false) {
  return (dispatch: () => void, getState) => {
    ...
    if (account && (settings.node || settings.node.length !== 0)) {
      ...
      let selectedTokens = ['picoio.token:' + settings.blockchain.tokenSymbol];
      ...
      forEach(selectedTokens, (namespace) => {
        ...
        pico(connection).getCurrencyBalance(contract, account, symbol).then((results) =>
          dispatch({
...
export function getAccountByKey(key) {
  return (dispatch: () => void, getState) => {
    ...
    if (key && (settings.node || settings.node.length !== 0)) {
      pico(connection).getKeyAccounts(key).then((accounts) => {
...
export function claimGBMRewards() {
  return (dispatch: () => void, getState) => {
    ...
    return pico2(connection, true).transact(op, {
      broadcast: true,
      blocksBehind: 3,
      expireSeconds: 120
    }).then((tx) => {

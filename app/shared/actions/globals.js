...
export function getGlobals() {
  return (dispatch: () => void, getState) => {
    ...
    pico(connection).getTableRows(true, 'picoio', 'picoio', 'global').then((results) => dispatch({
...
export function getCurrencyStats(contractName = "picoio.token", symbolName) {
  return (dispatch: () => void, getState) => {
    dispatch({
    ...
    pico(connection).getCurrencyStats(account, symbol).then((results) => {
      if (isEmpty(results)) {
        return dispatch({
...
export function getRamStats() {
  return (dispatch: () => void, getState) => {
    ...
    pico(connection).getTableRows(query).then((results) => {
      const { rows } = results;
    ...
    return await fetch(`${config.CARBON_ROOT}/v1/card/addNew`, {
      ...
      if (response.details && response.details.creditDebitId){
        ...
        let postBody = new URLSearchParams({
          ...
          memo: memo || '',
          contactId: contactId,
          confirmationUrl:`https://picowallet.io/carbon?action=confirm&contactid=${contactId}`,
          successRedirectUrl:`https://picowallet.io/carbon?action=success&contactid=${contactId}`,
          errorRedirectUrl:`https://picowallet.io/carbon?action=fail&contactid=${contactId}`
        });
...
export function create2FA(contactId, issuer='PicoWallet') {
  return async (dispatch: () => void, getState) => {
    dispatch({
...
export function getPriceFeedGecko(baseToken, quoteToken, actualToken) {
  return (dispatch: () => void, getState) => {
    ...
    return fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${baseToken}&vs_currencies=${quoteToken}`, 
    {
      ...
      const picoPrice = response[baseToken.toLowerCase()].pico;
      const usdPrice = response[baseToken.toLowerCase()].usd;
      const isUSDQuote = (quoteToken == "USD");

      const data = {
        price: isUSDQuote ? usdPrice : picoPrice,
...
export function getProfiles(account) {
  return (dispatch: () => void, getState) => {
    ...
    const { connection } = getState();
    const query = {
      scope: config.PICO_CONTRACT,
      code: config.PICO_CONTRACT,
      table: 'profiles',
      json: true,
      limit:1000000
    };
...
export function setProfileAvatar(avatar, bio) {
  return (dispatch: () => void, getState) => {
    ...
    return pico(connection, true, payforaction!==null).transaction({actions: actions})
    .then((tx) => {
      setTimeout(() => {
...
export function openProfile() {
  return (dispatch: () => void, getState) => {
    ...
    return pico(connection, true, true).transaction({actions: actions})
    .then((tx) => {
      setTimeout(() => {
        dispatch(getProfiles());
      }, 1000);
...
export function getCustomTokensRemote() {
  return (dispatch: () => void, getState) => {
    ...
    return fetch('https://raw.githubusercontent.com/telos-foundation/pico/master/resources/tokens.json')
    .then(response => response.json()).then((response) => {
      return dispatch({
        payload: response,

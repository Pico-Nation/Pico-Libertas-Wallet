...
const defaultContract = 'picoio.amend';

export function getRatifyAmends(scope = 'picoio.amend', previous = false) {
...
export function getVoteInfo(scope, account) {
  return (dispatch: () => void, getState) => {
    ...
    const formatted = pico(connection).modules.format.encodeNameHex(account, true)
...
export function unvoteProposal(scope, voter, proposal_name) {
  return (dispatch: () => void, getState) => {
    ...
    return pico(connection, true).transaction({
      actions: [
        {

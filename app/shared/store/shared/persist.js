...
const migrations = {
  ...
  */
  3: (state) => {
    ...
    if (
      ...
      newSettings.customTokens = ['picoio.token:'];
    }
    if (
      newSettings.customTokens
      && newSettings.customTokens.indexOf('picoio.token:') === -1
    ) {
      newSettings.customTokens.push('picoio.token:');
    }
 ...
 8: (state) => {
    ...
    /*if (
      !newSettings.blockchains
      || !newSettings.blockchains.length
      || newSettings.blockchains.filter(b => b.node==='https://api.pico.miami:17441').length > 0
    ) {*/

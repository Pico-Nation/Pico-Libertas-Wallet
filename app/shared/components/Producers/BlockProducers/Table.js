...
class ProducersTable extends Component<Props> {
  ...
  render() {
    ...
    const coreToken = contract['picoio.token'] && contract['picoio.token'][coreSymbol];
    let tokenSupply = coreToken && coreToken.supply ? parseFloat(coreToken.supply) : 0;
    tokenSupply = isPICOChain ? tokenSupply : tokenSupply - 140279973.0000; // telos supply mins 140279973.0000 exchange pool
    ...
    return (
      <Segment basic loading={loading} vertical>
        ...
        <Grid>
          <Grid.Column width={8}>
            <Header size="small">
              {activatedStake.toLocaleString()} {t('block_producer_pico_staked', {tokenSymbol:settings.blockchain.tokenSymbol})} ({activatedStakePercent}%)
              <Header.Subheader>
                <ProducersVoteWeight
                  settings={settings}

...
class ToolsPermissions extends Component<Props> { 
  ...
  componentDidMount() {
    ...
    actions.getAbi('picoio');

    pico(connection).getActions(settings.account, -1, -100000).then((results) => {
  ...
  isValidContract = (name) => {
    ...
    return (
      contracts[name]
      && contracts[name] instanceof PICOContract
    );
  }
  render() {
    const {
    ...
    let contract = null;
    if (contracts && contracts.picoio && contracts.picoio.abi)
      contract = new PICOContract(contracts.picoio.abi, 'picoio');
    ...
    let authorization = new PICOAccount(account).getAuthorization(pubkey, true);

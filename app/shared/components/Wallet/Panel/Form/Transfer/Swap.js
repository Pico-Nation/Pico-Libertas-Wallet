...
import pico from '../../../../../actions/helpers/pico';
...
class WalletPanelFormTransferSwap extends Component<Props> {
  constructor(props) {
    ...
    const toToken = globals.remotetokens && globals.remotetokens.filter((token) => token.symbol=='PICO')[0];

    this.state = {
      fromAsset: 'QBE',
      toAsset: 'PICO',
      quantity: '0.0000 QBE',
      toQuantity: '0.0000 PICO',
  ...
  async fetchConversionData (fromSymbol, toSymbol, editedField, _quantity) {
    ...
    const networkContract = 'picoio.token';
    ...
    if (fromToken.symbol != settings.blockchain.tokenSymbol) {
      fromSettings = await pico(connection).getTableRows(
        {
          json: true,
    ...
    if (toToken.symbol != settings.blockchain.tokenSymbol) {
      toSettings = await pico(connection).getTableRows(
        {
          json: true,
    ...
    if (fromToken.symbol != settings.blockchain.tokenSymbol) {
      fromCoreBalance = await pico(connection).getCurrencyBalance(
        networkContract, fromToken.bancor_converter, settings.blockchain.tokenSymbol);

      fromTokenBalance = await pico(connection).getCurrencyBalance(
    ...
    if (toToken.symbol != settings.blockchain.tokenSymbol) {
      toCoreBalance = await pico(connection).getCurrencyBalance(
        networkContract, toToken.bancor_converter, settings.blockchain.tokenSymbol);

      toTokenBalance = await pico(connection).getCurrencyBalance(

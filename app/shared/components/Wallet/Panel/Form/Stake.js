...
class WalletPanelFormStake extends Component<Props> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      decimalCpuAmount: 0,
      decimalNetAmount: 0,
      PICObalance: (props.balance && props.balance[settings.blockchain.tokenSymbol]) 
  ...
  errorsInForm = () => {
    ...
    if (activeTab == 'stake') {
      if (decimalCpuAmount.greaterThan(PICObalance) ||
        decimalNetAmount.greaterThan(PICObalance) ||
        decimalCpuAmount.plus(decimalNetAmount).greaterThan(PICObalance)
        )
      {
  ...
  render() {
    ...
    const PICObalance = balance[settings.blockchain.tokenSymbol] || 0;

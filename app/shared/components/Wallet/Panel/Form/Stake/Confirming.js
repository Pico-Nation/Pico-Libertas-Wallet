...
class WalletPanelFormStakeConfirming extends Component<Props> {
  ...
  render() {
    ...
    const lessThanOnePicoStaked = (cpuAmount < 1 || netAmount < 1);
    ...
    return (
      <div>
        ...
        <Segment padding="true" basic>
            ...
            {(!unstaking) ? (
              <Segment>
                <Header textAlign="center">
                  ...
                  <Header.Subheader>
                    ({t('you_will_have')} {netDifference.toFixed(settings.tokenPrecision)} {t('pico_in_net_after', {tokenSymbol:settings.blockchain.tokenSymbol})})
                  </Header.Subheader>
                </Header>
              </Segment>
            ) : ''}
            ...
            {(!unstaking) ? (
              <Segment>
                <Header textAlign="center">
                  ...
                  <Header.Subheader>
                    ({t('you_will_have')} {cpuDifference.toFixed(settings.tokenPrecision)} {t('pico_in_cpu_after', {tokenSymbol:settings.blockchain.tokenSymbol})})
                  </Header.Subheader>
                </Header>
              </Segment>
            ) : ''}
          ...
          {(accountName === account.account_name && lessThanOnePicoStaked) ? (
            <Message warning="true">{t('will_have_less_than_one_pico_staked', {tokenSymbol:settings.blockchain.tokenSymbol})}</Message>
          ) : ''}

...
let {ecc, Fcbuffer} = Pico.modules;
import ObjectHelpers from '../../util/ObjectHelpers';
import * as ricardianParser from 'pico-rc-parser';
...
import picojs2 from 'picojs2';
import * as numeric from  "../../../../node_modules/picojs2/dist/picojs2-numeric";
...
const getCachedInstance = network => {
    if(cachedInstances.hasOwnProperty(network.unique())) return cachedInstances[network.unique()];
        ...
        const pico = Pico({httpEndpoint:`${network.fullhost()}`, chainId:network.chainId});
        cachedInstances[network.unique()] = pico;
        return pico;
    }
}
...
const getAccountsFromPublicKey = (publicKey, network) => {
    return Promise.race([
        ...
        new Promise((resolve, reject) => {
            const pico = getCachedInstance(network);
            pico.getKeyAccounts(publicKey).then(res => {
                if(!res || !res.hasOwnProperty('account_names')){ resolve([]); return false; }

                Promise.all(res.account_names.map(name => pico.getAccount(name).catch(e => resolve([])))).then(multires => {
                    let accounts = [];
...
export default class PICO{
    ...
    constructor(){ 
        this.blockchain = {
            chain: Blockchains.PICOIO,
            tokenSymbol: Blockchains.PICOIO.toUpperCase(),
            chainId: "",
            node: "",
        }
        //this.name = Blockchains.TELOS;
        this.name = Blockchains.PICOIO;
    ...
    async isEndorsedNetwork(network){
        ...
        return network.blockchain === Blockchains.PICOIO && network.chainId === endorsedNetwork.chainId;
    }

    async getChainId(network){
        const pico = Pico({httpEndpoint:`${network.protocol}://${network.hostport()}`});
        return pico.getInfo({}).then(x => x.chain_id || '').catch(() => '');
    }
    ...
    //     const pico = Pico({httpEndpoint:network.fullhost(), chainId:network.chainId, signProvider});
    //     return await pico.delegatebw(account.name, account.name, '0.0000 PICO', '0.1000 PICO', 0, { authorization:[account.formatted()] })
    ...
    privateToPublic(privateKey, prefix = null){ return ecc.PrivateKey(privateKey).toPublic().toString(prefix ? prefix : Blockchains.PICOIO.toUpperCase()); }
    validPrivateKey(privateKey){ return privateKey.length === 51 && ecc.isValidPrivate(privateKey); }
    validPublicKey(publicKey, prefix = null){ return ecc.PublicKey.fromStringOrThrow(publicKey, prefix ? prefix : Blockchains.PICOIO.toUpperCase()); }
    ...
    //     const pico = getCachedInstance(account.network());
    //     return Promise.race([
    //         new Promise(resolve => setTimeout(() => resolve(null), 2000)),
    //         pico.getAccount(account.name)
    //     ])
    // }
    ...
    //     const balances = await pico.getTableRows({
    //         json:true,
    //         code:tokenAccount,
    //         scope:account.name,
    //         table:'accounts',
    ...
    defaultToken(){ 
        return {symbol:this.blockchain.tokenSymbol, account:'picoio.token', name:'Telos', blockchain:this.blockchain.chain}; 
    }

    // async fetchTokens(tokens){
    //     tokens.push(this.defaultToken());
    //     const picoTokens = await fetch("https://raw.githubusercontent.com/telos-foundation/pico/master/resources/tokens.json").then(res => res.json()).catch(() => []);
    //     picoTokens.map(token => {
    //         token.blockchain = Blockchains.PICOIO;
    ...
    //         const pico = Pico({httpEndpoint:network.fullhost(), chainId:network.chainId, signProvider});
    //         if(staking) resolve(pico.delegatebw(account.name, account.name, net, cpu, 0, { authorization:[account.formatted()] })
    //             .catch(error => ({error:JSON.parse(error).error.details[0].message.replace('assertion failure with message:', '').trim()}))
    //             .then(res => res));

    //         else resolve(pico.undelegatebw(account.name, account.name, net, cpu, { authorization:[account.formatted()] })
    //             .catch(error => ({error:JSON.parse(error).error.details[0].message.replace('assertion failure with message:', '').trim()}))
    //             .then(res => res));
    //     })
    // }
    ...
    //         const pico = Pico({httpEndpoint:network.fullhost(), chainId:network.chainId, signProvider});
    //         if(buying) resolve(pico.buyrambytes(account.name, account.name, bytes, { authorization:[account.formatted()] })
    //             .catch(error => ({error:JSON.parse(error).error.details[0].message.replace('assertion failure with message:', '').trim()}))
    //             .then(res => res));

    //         else resolve(pico.sellram(account.name, bytes, { authorization:[account.formatted()] })
    //             .catch(error => ({error:JSON.parse(error).error.details[0].message.replace('assertion failure with message:', '').trim()}))
    //             .then(res => res));
    //     })
    // }
    ...
    //         const pico = Pico({httpEndpoint:account.network().fullhost(), chainId:account.network().chainId, signProvider});
    //         const contractObject = await pico.contract(contract);
    ...
    async createTransaction(actions, account, network){
        ...
        const options = {
            ...
            broadcast: false,
            keyPrefix: Blockchains.PICOIO.toUpperCase(),
            sign: true,
            signProvider
        };

        const contractNames = actions.map(x => x.contract);
        const pico = Pico(options);

        await pico.transaction(contractNames, contracts => {
            actions.map(action => {
                try {
    ...
    async getAbis(contracts, network, pico){
        const abis = {};
        ...
        await Promise.all(contracts.map(async contractAccount => {
            ...
            // if(cachedABI === 'object' && cachedABI.timestamp > +new Date((await pico.getAccount(contractAccount)).last_code_update))
                // abis[contractAccount] = pico.fc.abiCache.abi(contractAccount, cachedABI.abi);

            // else {
                abis[contractAccount] = (await pico.contract(contractAccount)).fc;
    ...
    async parsePicojsRequest(payload, network){
        ...
        const abis = await this.getAbis(contracts, network, pico);
    ...
    async parsePicojs2Request(payload, network){
		...
		const rpc = new picojs2.Rpc.JsonRpc(network.fullhost());
		const api = new picojs2.Api({rpc});
		...
		const abis = await Promise.all(contracts.map(async accountName => {
			const cachedABI = null; // await StorageService.getCachedABI(accountName+'picojs2', network.chainId);
			...
			const fetchAbi = async () => {
				...
				/*await StorageService.cacheABI(accountName+'picojs2', network.chainId, {
					rawAbiHex,
					timestamp:+new Date()
				});*/
			};

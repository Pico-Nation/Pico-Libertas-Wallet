...
import * as PicoTypes from '../../shared/actions/types';
import pico from '../../shared/actions/helpers/pico';
...
export function unlockWallet(data) {
    return async (dispatch: () => void, getState) => {
        ...
        dispatch({
            type: PicoTypes.VALIDATE_WALLET_PASSWORD_PENDING
        });

        let account = accounts[wallet.account];
        if (settings.walletMode === 'hot' && !account) {
            account = await pico(connection).getAccount(wallet.account);
        }
        ...
        return new Promise((resolve)=>{
            setTimeout(()=>{
                ...
                if (ecc.isValidPrivate(key) === true) {
                    if(!changeWallet){
                        dispatch({
                          type: PicoTypes.VALIDATE_WALLET_PASSWORD_SUCCESS
                        });
                        return resolve(key);
                    }
                    ...
                    dispatch({
                      ...
                      type: PicoTypes.SET_WALLET_ACTIVE
                    });
                    ...
                    dispatch({
                      ...
                      type: PicoTypes.SET_WALLET_KEYS_ACTIVE
                    });
                }else{
                    key = '';
                }
                ...
                if(!key){
                    dispatch({
                        type: PicoTypes.VALIDATE_WALLET_PASSWORD_FAILURE
                    });
                }else{
                    dispatch({
                        type: PicoTypes.VALIDATE_WALLET_PASSWORD_SUCCESS
